...
def save_labels(labels):
    '''
    INPUT list 'labels': filepaths to all labels
    '''
    progress.currval = 0
    for label_idx in progress(range(len(labels))):
        slices = io.imread(labels[label_idx], plugin = 'simpleitk')
        for slice_idx in range(len(slices)):
            io.imsave('/Users/yuan/Documents/LM/Training/Labels/{}_{}L.png'.format(label_idx, slice_idx), slices[slice_idx])


if __name__ == '__main__':
    labels = glob('/Users/yuan/Documents/LM/Training/HGG/**/*more*/**.mha')
    save_labels(labels)
